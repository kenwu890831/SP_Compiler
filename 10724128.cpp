...
void Syntax::ErrorTest() {
        if ( table6.size() >20 ) {
        if ( table6.at(10).n5 != 0 )
        table6.at(10).n6 -= 1 ;
        if ( table6.at(10).n3 != 0 )
        table6.at(10).n4 -= 1 ;
        if ( table6.at(19).n3 != 0 )
        table6.at(19).n4 -= 1 ;
        }
    ...
    while ( i < endLine ) { // endLine 為換行所在位址
    	...
    	if ( token.at(i).tableType == 2 ) { // reserve word table
			...
			if ( all.at(index).error == false && countLeft != countRight ) {
				all.at(index).error = true ;
				all.at(index).errormsg = "Incorrect number of square brackets in assignment" ;
				break ;
			} // if
			break ;
